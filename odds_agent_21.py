...
class OddsAgentV21(object):
    ...
    def __init__(self, num_actions, threshold_offset={
        ...
        self.para_all = {
           'preflop':{
               ...
               # mixing call vs raise, when equity is enought (equity > bet-over-port)
               ,'pct_equity_enough_raise': 0.4 # rest are all call
               }
           ,'flop':{
               # threshold in hand strength
    ...
    def step_generic_para(self, state, step_no):
        ...
        ## If other ppl raised into me
            ...
            #   Strictly calculate port odds facing a bet
            
            port_size = sum(state['raw_obs']['all_chips'])
            bet_amt = list(filter(lambda x: x != state['raw_obs']['my_chips'], state['raw_obs']['all_chips']))[0] - state['raw_obs']['my_chips']
            assert bet_amt > 0
            bet_amt_pct_port = bet_amt/port_size            
            
            ## (1) If we have enough equity for bet-over-port, either raise or call
            if decision_value > bet_amt_pct_port:
                # raise at a freq of equity => WIP
                if random.uniform(0, 1) < decision_value:
                    return 'raise' if 'raise' in state['raw_legal_actions'] else 'call'
                else:
                    return 'call'
            
            ## If we do not have enough equity, mostly fold
            #   but we mix fold with call or fold
            else:
